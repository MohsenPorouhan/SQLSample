/* Formatted on 8/23/2020 3:38:36 ب.ظ (QP5 v5.287) */
  SELECT EMPLOYEE_NUMBER,
    ...
    FROM (  SELECT APPROVAL_TIMECARD, EMPLOYEE_NUMBER,
                   ...
                   START_DATE,
                   CASE
                      WHEN CHARGE_TYPE = 'کار عادي' THEN SUM (MEASURE)
                      ELSE 0
                   END
                      hour_addi,
                   CASE
                      WHEN CHARGE_TYPE = 'اضافه کار' THEN SUM (MEASURE)
                      ELSE 0
                   END
                      hour_over,
                   CASE
                      WHEN CHARGE_TYPE = 'ماموريت درون استان'
                      THEN
                         SUM (MEASURE)
                      ELSE
                         0
                   END
                      hour_daroon,
                   CASE
                      WHEN CHARGE_TYPE = 'ماموريت خارج از کشور'
                      THEN
                         CEIL (SUM (MEASURE) / 24)
                      ELSE
                         0
                   END
                      day_kharej_mis,
                   CASE
                      WHEN CHARGE_TYPE = 'ماموريت اداري'
                      THEN
                         CEIL (SUM (MEASURE) / 24)
                      ELSE
                         0
             ...
             WHERE     START_DATE2 BETWEEN NVL ( :p_start, '13980101')
                   ...
                   -- and CHARGE_TYPE in ('کار عادي','اضافه کار','ماموريت درون استان')
   ...
   WHERE V.PERSON_ID IN
            (SELECT PERSON_ID
              ...
              WHERE        PAA.PERSON_ID = V.PERSON_ID
                    ...
                    OR PAa.ASS_ATTRIBUTE20 IN (:p_GROUPS) or (:P_GROUPS is null and PAA.ASS_ATTRIBUTE20 in(SELECT organization_id
                                FROM hr_organization_units
                               WHERE     organization_id IN
                                            (    SELECT pose.organization_id_child
                                                   FROM (SELECT DISTINCT
