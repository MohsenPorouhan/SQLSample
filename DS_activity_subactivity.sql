SELECT   distinct per_grade.employee_number employee_number,--Per_Time_PRJ_TASK.ASSIGNMENT_NUMBER employee_number,/*13980516*/
         ...
         (case when 2 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.InMissionTime), 2) else 0 end)         
         + (CASE
         ...
         (case when 1 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.overtime * FER.OVER_TIME_RATE)) else 0 end)
            overtime_EXPENSE,
         (case when 2 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.InMissionTime * FER.DAROON_OSTAN)) else 0 end)           
         + (CASE
