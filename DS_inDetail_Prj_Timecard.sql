SELECT   per_grade.employee_number employee_number,--Per_Time_PRJ_TASK.ASSIGNMENT_NUMBER employee_number,
         per_grade.full_name,
         org.D_child_NAME,
      ...
      --   to_char(Per_Time_PRJ_TASK.time_period,'rrrrmmdd','nls_calendar=persian') datete,
         ...
         (case when 0 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.NormalTime), 2) else 0 end) Normal_Time,
         (case when 1 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.overtime), 2) else 0 end)  overtime,
         (case when 2 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.InMissionTime), 2) else 0 end) inmission,         
         (CASE
               ...
               ELSE
                  0
            END) outmission,
         (case when 4 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.ForeignMissionTime), 2) else 0 end)         
            foreignmission,
         --0 sum_time_sub_activity,
         (case when 0 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.NormalTime * FER.ADDI_RATE)) else 0 end)
            NORMAL_TIME_EXPENSE,
         (case when 1 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.overtime * FER.OVER_TIME_RATE)) else 0 end)
            overtime_EXPENSE,
         (case when 2 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.InMissionTime * FER.DAROON_OSTAN)) else 0 end)  inmission_expense,       
         (CASE
               ...
               ELSE
                  0
            END) outmission_expense,
         (case when 4 in (:work_type_code) then ROUND (SUM (Per_Time_PRJ_TASK.ForeignMissionTime * FER.MISSION_FO)) else 0 end)         
            foreignmission_expense,
...
GROUP BY per_grade.employee_number,--Per_Time_PRJ_TASK.ASSIGNMENT_NUMBER,
         full_name,         
         D_child_NAME,
   ...
   --      Per_Time_PRJ_TASK.time_period
-- HAVING (
-- (case when 0 in (:work_type_code) then (SUM (Per_Time_PRJ_TASK.NormalTime)) else 0 end) +
--         (case when 1 in (:work_type_code) then (SUM (Per_Time_PRJ_TASK.overtime)) else 0 end)  +
--         (case when 2 in (:work_type_code) then (SUM (Per_Time_PRJ_TASK.InMissionTime)) else 0 end)         
--         + (CASE
--               WHEN :mission_work_type_code IN (31,
--                                                32,
--                                                33,
...
--                  0
--            END)
--         + (case when 4 in (:work_type_code) then SUM (Per_Time_PRJ_TASK.ForeignMissionTime) else 0 end) <> 0 )
ORDER BY (activity), wbs_level, per_grade.employee_number -- Per_Time_PRJ_TASK.ASSIGNMENT_NUMBER